...
def create_manual_realignment_proj_data_gif(dir_path,
    ...
    vmin_xrt = np.min([intensity_xrt_norm, shifted_intensity_xrt_norm])
    vmax_xrt = np.max([intensity_xrt_norm, shifted_intensity_xrt_norm])
    ...
    im1_1 = axs[0].imshow(intensity_ref_element[0], vmin = vmin, vmax = vmax)
    im1_2 = axs[1].imshow(intensity_xrt_norm[0], vmin = vmin_xrt, vmax = vmax_xrt)
    ...
    axs[0].set_title(r'xrt_sig (man. aligned)', fontsize = 16)
    axs[1].set_title(r'{0} (man. aligned)'.format(ref_element), fontsize = 16)
...
def create_cor_fig(dir_path, raw, aligned, element_of_interest, theta, desired_theta_pair_idx, element_of_interest_override = None):
    fig, axs = plt.subplots(2, 3)

    if element_of_interest_override is not None:
        element_of_interest = element_of_interest_override
...
def run_mpv(filename):
    if sys.platform == 'linux':
        ...
        subprocess.run(['mpv', '--fs', '--keep-open=yes', '--linear-downscaling=no', filename], stdout = subprocess.DEVNULL, stderr = subprocess.DEVNULL)
