...
def create_aggregate_xrf_h5(file_path_array, output_h5_file, synchrotron):
    ...
    elements, counts, theta, nx, ny, dx_cm, dy_cm = extract_h5_xrf_data(file_path_array[0], synchrotron) # Invoke the first time for getting the number of elements and the number of pixels
    ...
    for theta_idx, file_path in enumerate(file_path_array):
        elements_new, counts, theta, nx_new, ny_new, dx_cm, dy_cm = extract_h5_xrf_data(file_path, synchrotron)
