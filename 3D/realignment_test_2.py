...
# If no initial shift(s) given or scalar shifts given
    ...
    for cor_iter in range(max_cor_iterations):
        ...
        if abs(offset) < eps_cor:
            ...
            for theta_idx in range(n_theta):
                aligned_proj[theta_idx] = ndi.shift(xrf_proj_img_array[ref_element_idx, theta_idx], shift = (0, -(net_offset - 4.8)))
    ...
    net_x_shifts_pc[0] -= (net_offset - 4.8)
    # for theta_idx in range(n_theta):
...
output_file_name_base = 'xrt_gridrec_6_iter_dynamic_ps_cor_correction_no_log_w_padding_07_29_2025'
