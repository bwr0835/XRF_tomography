...
i_tot_xrf = np.array([i_tot_xrf_0126, i_tot_xrf_0125, i_tot_xrf_0124])
i_tot_us_ic = np.array([i_tot_us_ic_0126, i_tot_us_ic_0125, i_tot_us_ic_0124])
...
result = mod.fit(i_tot_xrf, params, x = dz)

a = result.params['A'].value
a = f"{a:.2e}"
a_coeff, a_exponent = a.split("e+0")
...
plt.scatter(dz, i_tot_xrf, color = 'black')
plt.plot(x1, yfit, 'r--', label = r'$I = {0}\times 10^{1}/\left({2} + \Delta d\right)^{{2}}$'.format(a_coeff, a_exponent, round_correct(z_det_nom, ndec = 1)))
