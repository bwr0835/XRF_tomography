...
_, counts_fine, _, x_um_fine, y_um_fine, nx_fine, ny_fine, dx_cm_fine, dy_cm_fine = util.extract_h5_xrf_data(fine_scan_filename, synchrotron = 'nsls-ii', scan_coords = True)
...
x0_coarse_idx, y0_coarse_idx = np.unravel_index(np.argmin(np.abs(x_um_coarse - x0_fine)), x_um_coarse.shape).astype(int), np.unravel_index(np.argmin(np.abs(y_um_coarse - y0_fine)), y_um_coarse.shape)
