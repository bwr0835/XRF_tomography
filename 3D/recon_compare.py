...
for slice_idx in range(n_slices):
    ...
    im1_1.set_data(recon_gridrec_no_shift[slice_idx], norm = colors.Normalize(vmin = np.quantile(recon_gridrec_no_shift[slice_idx], [0.02, 0.98])[0], vmax = np.quantile(recon_gridrec_no_shift[slice_idx], [0.02, 0.98])[1]))
...
for theta_idx in range(n_theta):
    ...
    im2_2.set_data(synth_proj_array_gridrec_no_shift[theta_idx], norm = colors.Normalize(vmin = np.quantile(synth_proj_array_gridrec_no_shift[theta_idx], [0.02, 0.98])[0], vmax = np.quantile(synth_proj_array_gridrec_no_shift[theta_idx], [0.02, 0.98])[1]))
    im2_3.set_data(synth_proj_array_mlem_no_shift[theta_idx], norm = colors.Normalize(vmin = np.quantile(synth_proj_array_mlem_no_shift[theta_idx], [0.02, 0.98])[0], vmax = np.quantile(synth_proj_array_mlem_no_shift[theta_idx], [0.02, 0.98])[1]))
