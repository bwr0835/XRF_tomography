...
dir_path = '/Users/bwr0835/Documents/2_ide_realigned_data_04_04_2026_cor_correction_iter_reproj/xrt_od_xrf_realignment'

shift_file = os.path.join(dir_path, 'aux_data', 'dx_iter_array.npy')
theta_file = os.path.join(dir_path, 'output_net_shift_data.csv')

# aligned_proj_array = np.load(aligned_proj_file)
dx_iter_array = np.load(shift_file)
theta_array = pd.read_csv(theta_file)['theta_deg'].to_numpy().astype(float)

n_theta = len(theta_array)
n_iter = dx_iter_array.shape[0]
